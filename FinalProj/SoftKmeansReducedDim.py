...
if __name__ == "__main__":
    ...
    soft_kmeans_pca_2d = SoftKMeans(X_2d, y, k=3, beta=2)
    total_accuracy = []
    for seed in range(100):
        total_accuracy.append(soft_kmeans_pca_2d.run(seed))
    print(f"PCA 2D Average Accuracy: {np.mean(total_accuracy) :.2f}%  Var: {np.var(total_accuracy) :.2f}")
    # PCA 3维数据
    pca_3d = PCA(n_components=3)
    pca_3d.fit(X)
    X_3d = pca_3d.transform(X)
    soft_kmeans_pca_3d = SoftKMeans(X_3d, y, k=3, beta=2)
    total_accuracy = []
    for seed in range(100):
        total_accuracy.append(soft_kmeans_pca_3d.run(seed))
    ...
    soft_kmeans_ae_2d = SoftKMeans(X_encoded_2d, y, k=3, beta=2)
    total_accuracy = []
    for seed in range(100):
        total_accuracy.append(soft_kmeans_ae_2d.run(seed))
    ...
    soft_kmeans_ae_3d = SoftKMeans(X_encoded_3d, y, k=3, beta=2)
    total_accuracy = []
    for seed in range(100):
        total_accuracy.append(soft_kmeans_ae_3d.run(seed))
